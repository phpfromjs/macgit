...
if(!empty($allidlist)){
$rs="select * from ".$data['video']." where id in($allidlist)";
...
$sql = "delete from ".$data['video']." where id in($allidlist)"; 
mysql_query($sql,$conn) or die("ERROR: ".mysql_error());
echo "<script language='javascript'> window.alert('删除对象成功！'); location='VideoList.php';</script>";
exit;
}
}
?>
<?php
if(!empty($id)){
$rs="select * from ".$data['video']." where id='".$id."'";
$resul=mysql_query($rs,$conn) or die("ERROR: ".mysql_error());
while($title=mysql_fetch_array($resul)){
$spic="../../".$title['spic'];
$bpic="../../".$title['bpic'];
...
$sql = "delete from ".$data['video']." where id='".$id."'"; 
mysql_query($sql,$conn) or die("ERROR: ".mysql_error());
echo "<script language='javascript'> window.alert('删除对象成功！'); location='VideoList.php';</script>";
exit;
}
