...
?>
<?php
$sql="select * from ".$data['video']." order by sort desc";
//echo $sql;exit();
...
<body>
    ...
    <form method="POST" name="myform" onSubmit="return CheckForm();" action="VideoSave.php?action=add" target="_self">
    <ul class="forminfo">
    <li><label>所属栏目</label><select name="classid" class="dfinput" style="width:180px;"><?php ShowTree(0,$conn,$sjk)?></select>&nbsp;&nbsp;&nbsp;&nbsp;排序：<input name="sort" id="sort" type="text" value="<?php echo $maxsort;?>" onBlur="javascript:if(isNaN(this.value)){alert('必须是数字!');this.value=<?php echo $maxsort;?>;}" class="dfinput" style="width:100px;" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;点击数：
                <input name="hits" type="text" id="hits" value="0" class="dfinput" style="width:100px;" /><i></i></li>
    
    <li><label>栏目名称</label><input name="title" id="title" type="text" class="dfinput" /><i></i></li>
    ...
    <li style="display:none"><label>栏目型号</label><input name="model" id="model" type="text" class="dfinput" /><i></i></li>
    ...
    <li><label>缩略图</label><input name="spic" id="spic" type="hidden" class="dfinput" /><iframe style="top:2px" id="UploadFiles" src="../upfile/upload.php?formname=myform&editname=spic&uppath=pic&filelx=jpg" frameborder="0" scrolling="No" width="280" height="30"></iframe><i>荣誉资质：550 X 733px ； 厂房厂貌：550 X 360px</i></li>
    
    <li style="display:none"><label>栏目大图</label><input name="bpic" id="bpic" type="hidden" class="dfinput" /><iframe style="top:2px" id="UploadFiles" src="../upfile/upload.php?formname=myform&editname=bpic&uppath=pic&filelx=jpg" frameborder="0" scrolling="No" width="280" height="30"></iframe><i>（英文）尺寸:600px X 392px</i></li>

	<?php 
      for($i=1;$i<=4;$i++){
    ?>
    <li id="myTR_" style="display:none"><label>更多图片</label><input name="pic_<?php echo $i;?>" id="pic_<?php echo $i;?>" type="hidden" class="dfinput" /><iframe style="top:2px" id="UploadFiles" src="../upfile/upload.php?formname=myform&editname=pic_<?php echo $i;?>&uppath=pic&filelx=jpg" frameborder="0" scrolling="No" width="280" height="30"></iframe><i>尺寸:293px X 152px</i>
    ...
    <li style="display:none"><label>栏目详情<i></i></label><textarea name="content" id="content" cols="" rows="" class="textinput2"></textarea></li>
