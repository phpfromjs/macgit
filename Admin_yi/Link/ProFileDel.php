...
if (!empty($_GET['id']))
{
    $sql = "SELECT * FROM ".$data['link']." WHERE qy=".$_SESSION['qy']." AND id=".$_GET['id'] ;
    ...
    if (mysql_num_rows($resul) != 0)
    {
        if (!empty($_GET['fld']))
        {
            ...
            if ($_GET['fld'] == "spic")
            {
                $sqll = "UPDATE ".$data['link']." SET spic='$hpic' WHERE qy=".$_SESSION['qy']." AND id=".$_GET['id'];
            }
            else
            {
                $sqll = "UPDATE ".$data['link']." SET ".$_GET['fld']."='$hpic' WHERE qy=".$_SESSION['qy']." AND id=".$_GET['id'];
            }
