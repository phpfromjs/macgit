...
if (!empty($_GET['id']))
{
    $sql = "SELECT * FROM ".$data['wapinfo']." WHERE id=".$_GET['id'];
    ...
    if (mysql_num_rows($resul) != 0)
    {
        if (!empty($_GET['fld']))
        {
            ...
            {
                $sqll = "UPDATE ".$data['wapinfo']." SET wechatcode='$hpic' WHERE id=".$_GET['id'];
            }
            elseif(($_GET['fld'] == "selfwechatcode"))
            {
                $sqll = "UPDATE ".$data['wapinfo']." SET selfwechatcode='$hpic' WHERE id=".$_GET['id'];
            }
