...
if (!empty($_GET['pID']))
{
    $id = $_GET['pID'];
    $page = $_GET['page'];
    $sql = "SELECT * FROM ".$data['gas']." WHERE id='$id'";
...
<div class="formbody">
    ...
    <form method="POST" name="myform" onSubmit="return CheckForm();" action="GasSave.php?action=Modify" target="_self">
    <ul class="forminfo">
        <li><label>所属类别</label><select name="classid" class="dfinput" style="width:180px;"><?php ShowTree(0,$conn,$sjk)?></select><i></i></li>
        <li><label>文章排序</label><input type="text" id="sort" name="sort" value="<?php echo $res['sort'];?>" onBlur="javascript:if(isNaN(this.value)){alert('必须是数字!');this.value=<?php echo $res['sort'];?>;}" class="dfinput" style="width:100px;" /><i></i></li>
        <li><label>浏览次数</label><input type="text" id="hits" name="hits" value="<?php echo $res['hits'];?>" class="dfinput" style="width:100px;" /><i></i></li>
        <li><label>文章名称</label><input type="text" id="title" name="title" value="<?php echo $res['title'];?>" class="dfinput" /><i>（中文）</i></li>
        <li style="display:none;"><label>文章名称</label><input type="text" id="title_en" name="title_en" value="<?php echo $res['title_en'];?>" class="dfinput" /><i>（英文）</i></li>
        <li>
            <label>缩略图</label><input type="hidden" id="cls_imgX" name="cls_imgX" value="../../<?php echo $res['spic'];?>" size="53" class="tx1" />
        ...
        <li style="display:none;">
            <label>缩略图</label><input type="hidden" id="cls_imgD" name="cls_imgD" value="../../<?php echo $res['bpic'];?>" size="53" class="tx1" />
        ...
        <li><label>作者</label><input type="text" id="author" name="author" value="<?php echo $res['author'];?>" class="dfinput" /><i>（中文）</i></li>
        ...
        <li><label>文章摘要</label><textarea id="text" name="text" cols="" rows="" class="textinput"><?php echo $res['text'];?></textarea><i>（中文）</i></li>
        <li style="display:none;"><label>文章摘要</label><textarea id="text_en" name="text_en" cols="" rows="" class="textinput"><?php echo $res['text_en'];?></textarea><i>（英文）</i></li>
        <li><label>文章内容<i><br />（中文）</i></label><textarea id="content" name="content" cols="" rows="" class="textinput2"><?php echo $res['content'];?></textarea></li>
        <li style="display:none;"><label>文章内容<i><br />（英文）</i></label><textarea id="content_en" name="content_en" cols="" rows="" class="textinput2"><?php echo $res['content_en'];?></textarea></li>
        <li><b>SEO信息设置</b></li>
        ...
        <li><label>网页描述</label><textarea id="d" name="d" cols="" rows="" class="textinput"><?php echo $res['d'];?></textarea><i>（中文）</i></li>
        ...
        <li><label>推荐显示</label><input type="checkbox" id="tj" name="tj" value="1" <?php if ($res['tj'] != 0){echo "checked";}?> /><i>（如果选中的话将首页显示）</i></li>
        <li><label>置顶显示</label><input type="checkbox" id="isnew" name="isnew" value="1" <?php if ($res['isnew'] != 0){echo "checked";} ?> /><i>（如果选中的话将置顶显示）</i></li>
        ...
        <li><label>&nbsp;</label><input type="submit" name="" value="确认保存" class="btn" /><input type="hidden" id="pid" name="pid" value="<?php echo $res['id'];?>" /></li>
    </ul>
    </form>
</div>
<?php
