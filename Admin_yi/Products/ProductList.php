    ...
    switch($action)
    {
        case "tj";
            if (isset($_GET['sign']))
            {
                ...
                if ($sign == 1)
                {
                    mysql_query("UPDATE ".$data['pro']." SET tj=0 WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
                }
                else
                {
                    mysql_query("UPDATE ".$data['pro']." SET tj=1 WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
                }
            }
        ...
        case "isnew";
            if (isset($_GET['sign']))
            {
                ...
                if ($sign == 1)
                {
                    mysql_query("UPDATE ".$data['pro']." SET isnew=0 WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
                }
                else
                {
                    mysql_query("UPDATE ".$data['pro']." SET isnew=1 WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
                }
            }
            header("Location:".$_SESSION['url']."");
            break;
        case "zd";
            $sqlzd = "SELECT * FROM ".$data['pro']." ORDER BY sort DESC";
            ...
            mysql_query("UPDATE ".$data['pro']." SET sort=".$maxsort." WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            header("Location:".$_SESSION['url']."");
            break;
        case "sh";
            if (!empty($_GET['sign']))
            {
                ...
                if ($sign == "true")
                {
                    if (!empty($_GET['pid']))
                    {
                        mysql_query("UPDATE ".$data['pro']." SET passed=0 WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
                    }
                }
                else
                {
                    if (!empty($_GET['pid']))
                    {
                        mysql_query("UPDATE ".$data['pro']." SET passed=1 WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
                    }
                }
            }
            header("Location:".$_SESSION['url']."");
        ...
        case "move";
            ...
            if (!empty($_GET['pid']))
            {
                $resul = mysql_query("SELECT * FROM ".$data['pro']." WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            ...
            if (!empty($_GET['mid']))
            {
                $resull = mysql_query("SELECT * FROM ".$data['pro']." WHERE id='{$_GET['mid']}'", $conn) or die("ERROR: ".mysql_error());
            ...
            if (!empty($_GET['mid']))
            {
                if ($p_sort != -1)
                {
                    $px = "UPDATE ".$data['pro']." SET sort='{$p_sort}' WHERE id='{$_GET['mid']}'";
            ...
            if (!empty($_GET['pid']))
            {
                if ($m_sort != -1)
                {
                    mysql_query("UPDATE ".$data['pro']." SET sort='{$m_sort}' WHERE id='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
                }
            }
            header("Location:".$_SESSION['url']."");
            break;
...
$cid = $_GET['cid'];
$title = $_GET['title'];
$sql = "SELECT * FROM ".$data['pro']." WHERE id>0";
//分类条件
...
$_SESSION['url'] = "ProductList.php".$url;
...
<div class="rightinfo">
    <div class="tools">
        <form name="searchsoft" method="get" action="ProductList.php">
        <ul class="toolbar">
            <li class="click"><a href="ProductAdd.php"><span><img src="../images/t01.png" /></span>添加</a></li>
    ...
    <form name="del" method="Post" action="ProductDel.php" onSubmit="return ConfirmDel();">
...
else
{
    ...
    while ($title = mysql_fetch_array($resul))
    {
        ...
        $prev_id = 0;
        $next_id = 0;
        $rs = "SELECT * FROM ".$data['pro']." WHERE sort>".$title['sort'].$sqll." ORDER BY sort ASC";
        ...
        $rss = "SELECT * FROM ".$data['pro']." WHERE sort<".$title['sort'].$sqll." ORDER BY sort DESC";
...
?>
            <tr>
                ...
                <td>
<?php 
$sqlclass = "SELECT * FROM ".$data['classpro']." WHERE classid=".$title['classid']."";
...
if ($p_prv_id != 0)
{
    $sqlclass2 = "SELECT * FROM ".$data['classpro']." WHERE classid=".$p_prv_id."";
...
if ($cp_prv_id != 0)
{
    $sqlclass3 = "SELECT * FROM ".$data['classpro']." WHERE classid=".$cp_prv_id."";
...
?>
                ...
                <td>&nbsp;
                    ...
                    {
                    ?>
                    <a href="ProductList.php?action=move&pid=<?php echo $title['id'];?>&mid=<?php echo $prev_id;?>&page=<?php echo $page;?>">上移</a>
                    <?php
                    }
                    else
                    {
                    ...
                    {
                    ?>
                    <a href="ProductList.php?action=move&pid=<?php echo $title['id'];?>&mid=<?php echo $next_id;?>&page=<?php echo $page;?>">&nbsp;&nbsp;下移</a>
                    <?php
                    }
                    else
                    {
                ...
                <td>
                    ...
                    {
                    ?>
                    <a href="ProductList.php?action=sh&amp;sign=true&amp;pid=<?php echo $title['id'];?>">已审核</a>
                    <?php
                    }
                    else
                    {
                    ?>
                    <a href="ProductList.php?action=sh&amp;sign=false&amp;pid=<?php echo $title['id'];?>"><font style="color:#FF0000">未审核</font></a>
                    <?php
                    }
                    ?>
                </td>
                <td><a href="ProductModify.php?pID=<?php echo $title['id'];?>&page=<?php echo $page;?>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="ProductDel.php?ArticleID=<?php echo $title['id'];?>" onClick="return ConfirmDel();">删除</a></td>
            </tr>
<?php
    }
}
