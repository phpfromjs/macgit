...
?>
<?php
$sql = "SELECT * FROM ".$data['classpro']." ORDER BY sort DESC";
$resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
...
?>
    ...
    <form name="myform" method="post" action="ProductSort.php?action=add_myclass" onSubmit="return check();">
    <ul class="forminfo">
        ...
        <li ><label>分类图片</label><input type="hidden" id="class_img" name="class_img" size="53" class="tx1" /><iframe id="UploadFiles" src="../upfile/upload.php?formname=myform&editname=class_img&uppath=classimg&filelx=jpg" frameborder="0" scrolling="No" width="280" height="30" style="top:2px;"></iframe><i>（中文）尺寸:980px X 300px</i></li>
        <li><label>分类图片</label><input type="hidden" id="class_img_en" name="class_img_en" size="53" class="tx1" /><iframe id="UploadFiles" src="../upfile/upload.php?formname=myform&editname=class_img_en&uppath=classimg&filelx=jpg" frameborder="0" scrolling="No" width="280" height="30" style="top:2px;"></iframe><i>（英文）尺寸:980px X 300px</i></li>
        <li><b>SEO信息设置</b></li>
        <li><label>网页标题</label><input type="text" id="t" name="t" value="" class="dfinput" /><i>（中文）</i></li>
        <li ><label>网页标题</label><input type="text" id="t_en" name="t_en" value="" class="dfinput" /><i>（英文）</i></li>
        <li><label>网页关键字</label><input type="text" id="k" name="k" value="" class="dfinput" /><i>（中文）</i></li>
        <li ><label>网页关键字</label><input type="text" id="k_en" name="k_en" value="" class="dfinput" /><i>（英文）</i></li>
    ...
    case "Addnext";
        ...
        $all_id = $_GET['all_id'];
        $level = $_GET['level'];
	$px = "SELECT * FROM ".$data['classpro']." WHERE prv_id=".$prv_id." ORDER BY sort DESC";
    ...
    <form name="myform" method="post" action="ProductSort.php?action=add_myclassNext" onSubmit="return check();">
    ...
    <?php
        ...
        {
            $level = $_GET['level'];
            $sql = "SELECT * FROM ".$data['classpro']." WHERE classid=".$pid;
            $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            if (mysql_num_rows($resul) != 0)
            {
    ...
    <form name="myform" method="post" action="ProductSort.php?action=edit_myclass" onSubmit="return check();">
    ...
    <ul class="forminfo">
        ...
        <li ><label>分类名称</label><input type="text" id="class_name_en" name="class_name_en" value="<?php echo $rs['class_name_en'];?>" class="dfinput" /><i>（英文）</i></li>
        <li>
            <label>分类图片</label>
            ...
            <i>
                ...
                {
                ?>
                <a href="javascript:;" onMouseOver="document.getElementById('pic1').style.display='block';" onMouseOut="document.getElementById('pic1').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="ProductSort.php?action=del_files&pid=<?php echo $rs['classid'];?>&amp;fld=class_img" style="color:#FF0000;">删除</a>]
        ...
        <li >
            ...
            <i>
                ...
                {
                ?>
                <a href="javascript:;" onMouseOver="document.getElementById('pic2').style.display='block';" onMouseOut="document.getElementById('pic2').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="ProductSort.php?action=del_files&pid=<?php echo $rs['classid'];?>&amp;fld=class_img_en" style="color:#FF0000;">删除</a>]
        ...
        <li ><label>网页标题</label><input type="text" id="t_en" name="t_en" value="<?php echo $rs['t_en'];?>" class="dfinput" /><i>（英文）</i></li>
        <li><label>网页关键字</label><input type="text" id="k" name="k" value="<?php echo $rs['k'];?>" class="dfinput" /><i>（中文）</i></li>
        <li ><label>网页关键字</label><input type="text" id="k_en" name="k_en" value="<?php echo $rs['k_en'];?>" class="dfinput" /><i>（英文）</i></li>
        <li><label>网站描述</label><textarea id="d" name="d" cols="" rows="" class="textinput"><?php echo $rs['d'];?></textarea><i>（中文）</i></li>
        <li><label>网站描述</label><textarea id="d_en" name="d_en" cols="" rows="" class="textinput"><?php echo $rs['d_en'];?></textarea><i>（英文）</i></li>
        <li style="display:none;"><label>推荐显示</label><input type="checkbox" id="tj" name="tj" value="1" <?php if ($rs['tj'] != 0){echo "checked";} ?> /><i>（如果选中的话将置顶显示）</i></li>
        <li><label>&nbsp;</label><input type="submit" name="" value="确认保存" class="btn" />&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="" value="返&nbsp;&nbsp;回" class="btn" onclick="javascript:history.back(-1)" /></li>
    </ul>
    </form>
    <?php
    ...
    case "add_myclass";
        ...
        $sql = "SELECT * FROM ".$data['classpro']." WHERE class_name_cn='".$title."'";
        $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($resul) == 0)
        {
            $string="INSERT INTO ".$data['classpro']."(class_txt_cn,class_txt_en,class_txt_j,prv_id,all_id,class_name_cn,class_name_en,class_name_j,sort,class_img,class_img_en,class_img_j,p,t,t_en,t_j,k,k_en,k_j,d,d_en,d_j,tj) VALUES('$class_txt_cn','$class_txt_en','$class_txt_j','0','0','$title','$title_en','$title_j','$sort','$img','$img_en','$img_j','$p','{$t}','{$t_en}','{$t_j}','{$k}','{$k_en}','{$k_j}','{$d}','{$d_en}','{$d_j}','{$tj}')";
            mysql_query($string, $conn) or die("ERROR: ".mysql_error());
            $sq = "SELECT * FROM ".$data['classpro']." WHERE class_name_cn='".$title."'";
            ...
            if (mysql_num_rows($pp) != 0)
            {
                $temp = $tt['classid'];
                $temp2 = "|".$tt['classid']."|";
                $sq1 = "UPDATE ".$data['classpro']." SET all_id='$temp',link_id='$temp2' WHERE class_name_cn='".$title."'";
                mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
                echo "<script>alert('状态：添加成功');location.href='ProductSort.php';</script>";
            }
        }
        else
        {
    ...
    case "edit_myclass";
        ...
        $sq = "SELECT * FROM ".$data['classpro']." WHERE classid=".$pid;
        $pp = mysql_query($sq, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($pp) != 0)
        {
            $sq1 = "UPDATE ".$data['classpro']." SET class_txt_cn='$class_txt_cn',class_txt_en='$class_txt_en',class_txt_j='$class_txt_j',class_name_cn='$title',class_name_en='$title_en',class_name_j='$title_j',sort='$sort',class_img='$img',class_img_en='$img_en',class_img_j='$img_j',t='$t',t_en='$t_en',t_j='$t_j',k='$k',k_en='$k_en',k_j='$k_j',d='$d',d_en='$d_en',d_j='$d_j',p='$p',a1='$a1',a1_en='$a1_en',a2='$a2',a2_en='$a2_en',a3='$a3',a3_en='$a3_en',a4='$a4',a4_en='$a4_en',tj='$tj' WHERE classid=".$pid;
            mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
            echo "<script>alert('修改成功');location.href='ProductSort.php?xgprv_id=".$pid."';</script>";
        }
        else
        {
            echo "<script>alert('没有找到数据!');location.href='myclass.php?xgprv_id=".$pid."';</script>";
        }
        break;
    case "del_myclass";
        if (!empty($_GET['pid']))
        {
            //删除当前类
            $rs = "SELECT * FROM ".$data['classpro']." WHERE classid=".$_GET['pid'];
            ...
            $sql = "DELETE FROM ".$data['classpro']." WHERE classid=".$_GET['pid'];
            mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            //删除子类
            $rs = "SELECT * FROM ".$data['classpro']." WHERE prv_id=".$_GET['pid'];
            $resul = mysql_query($rs, $conn) or die("ERROR: ".mysql_error());
            while ($title = mysql_fetch_array($resul))
            {
                $spic = "../../".$title['class_img'];
            ...
            $sql = "DELETE FROM ".$data['classpro']." WHERE prv_id=".$_GET['pid'];
            mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            echo "<script language='javascript'>window.alert('删除对象成功！'); location='ProductSort.php?glprv_id=".$xgprv_id."';</script>";
        }
        else
        {
            echo "<script language='javascript'>window.alert('数据丢失！'); location='ProductSort.php?glprv_id=".$xgprv_id."';</script>";
        }
        break;
    case "mv_myclass";
        $p_sort = -1;
        ...
        if (!empty($_GET['pid']))
        {
            $resul = mysql_query("SELECT * FROM ".$data['classpro']." WHERE classid='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            $tt = mysql_fetch_array($resul);
            if (mysql_num_rows($resul) != 0)
            {
                $p_sort = $tt['sort'];
        ...
        if (!empty($_GET['mID']))
        {
            $resull = mysql_query("SELECT * FROM ".$data['classpro']." WHERE classid='{$_GET['mID']}'", $conn) or die("ERROR: ".mysql_error());
        ...
        if (!empty($_GET['mID']))
        {
            if ($p_sort != -1)
            {
                $px = "UPDATE ".$data['classpro']." SET sort='{$p_sort}' WHERE classid='{$_GET['mID']}'";
        ...
        if (!empty($_GET['pid']))
        {
            if ($m_sort != -1)
            {
                mysql_query("UPDATE ".$data['classpro']." SET sort='{$m_sort}' WHERE classid='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            }
        }
        header("Location:ProductSort.php?xgprv_id=".$_GET['pid']."");
        break;
    case "del_files";
        if (!empty($_GET['pid']) && is_numeric($_GET['pid']))
        {
            $sql = "SELECT * FROM ".$data['classpro']." WHERE classid=".$_GET['pid'];
            $resul = mysql_query($sql, $conn) or die("ERROR:".mysql_error());
            ...
            if (mysql_num_rows($resul) != 0)
            {
                if (!empty($_GET['fld']))
                {
                    ...
                    {
                        $sqll = "UPDATE ".$data['classpro']." SET class_img='$hpic' WHERE classid=".$_GET['pid'];
                    }
                    else
                    {
                        $sqll = "UPDATE ".$data['classpro']." SET ".$_GET['fld']."='$hpic' WHERE classid=".$_GET['pid'];
                    }
    ...
    case "add_myclassNext";
        ...
        $sql = "SELECT * FROM ".$data['classpro']." WHERE class_name_cn='".$title."' AND prv_id=".$prv_id;
        $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($resul) == 0)
        {
            $string = "INSERT INTO ".$data['classpro']."(class_txt_cn,class_txt_en,class_txt_j,prv_id,all_id,class_name_cn,class_name_en,class_name_j,sort,class_img,class_img_en,class_img_j,t,t_en,t_j,k,k_en,k_j,d,d_en,d_j,p,a1,a1_en,a2,a2_en,a3,a3_en,a4,a4_en,tj) VALUES('$class_txt_cn','$class_txt_en','$class_txt_j','$prv_id','$all_id','$title','$title_en','$title_j','$sort','$img','$img_en','$img_j','{$t}','{$t_en}','{$t_j}','{$k}','{$k_en}','{$k_j}','{$d}','{$d_en}','{$d_j}','{$p}','{$a1}','{$a1_en}','{$a2}','{$a2_en}','{$a3}','{$a3_en}','{$a4}','{$a4_en}','{$tj}')";
            mysql_query($string, $conn) or die("ERROR: ".mysql_error());
            $sq = "SELECT * FROM ".$data['classpro']." WHERE classid='".$prv_id."'";
            $pp = mysql_query($sq, $conn) or die("ERROR: ".mysql_error());
            $tt = mysql_fetch_array($pp);
            if (mysql_num_rows($pp) != 0)
            {
            ...
            $sqd = "SELECT * FROM ".$data['classpro']." ORDER BY classid DESC";
            ...
            if (mysql_num_rows($ppp) != 0)
            {
                $temp2 = $ttt['classid'];
                $templink = $temp.$temp2."|";
                $sq1 = "UPDATE ".$data['classpro']." SET link_id='$templink' WHERE classid=".$temp2;
                mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
                echo "<script>alert('状态：添加成功');location.href='ProductSort.php?glprv_id=".$prv_id."';</script>";
            }
        }
        else
        {
