...
if (!empty($_GET['id']))
{
    $sql = "SELECT * FROM ".$data['logistics']." WHERE id=".$_GET['id'];
    ...
    if (mysql_num_rows($resul) != 0)
    {
        if (!empty($_GET['fld']))
        {
            ...
            if ($_GET['fld'] == "spic")
            {
                $sqll = "UPDATE ".$data['logistics']." SET spic='$hpic' WHERE id=".$_GET['id'];
            }
            else
            {
                $sqll = "UPDATE ".$data['logistics']." SET ".$_GET['fld']."='$hpic' WHERE id=".$_GET['id'];
            }
