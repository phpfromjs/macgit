...
if (!empty($_GET['pID']))
{
    $id = $_GET['pID'];
    $page = $_GET['page'];
    $sql = "SELECT * FROM ".$data['logistics']." WHERE id='$id'";
...
<div class="formbody">
    ...
    <form method="POST" name="myform" onSubmit="return CheckForm();" action="logisticsSave.php?action=Modify">
    <ul class="forminfo">
        ...
        <li style="display:none;">
            ...
            <select name="brandid" class="dfinput" style="width:180px;">
                ...
                if ($rowclass < 1)
                {
                    echo "<option value=''>暂无品牌</option>";
                }
                else
                {
                    echo "<option value=''>选择品牌</option>";
                    while ($rsclass = mysql_fetch_array($resultclass))
                    {
                ?>
                <option value="<?php echo $rsclass["classid"];?>" <?php if($res['brandid']==$rsclass["classid"]){echo "selected";} ?>><?php echo $rsclass["class_name_cn"]."";?></option>
                <?php
                    }
                }
                ?>
        ...
        <li><label>产品名称</label><input type="text" id="title" name="title" value="<?php echo $res['title'];?>" class="dfinput" /><i>（中文）</i></li>
        <li ><label>产品名称</label><input type="text" id="title_en" name="title_en" value="<?php echo $res['title_en'];?>" class="dfinput" /><i>（英文）</i></li>
        <li ><label>价格</label><input type="text" id="price" name="price" value="<?php echo $res['price'];?>" class="dfinput" /><i>（中文）</i></li>
        <li><label>价格</label><input type="text" id="price1" name="price1" value="<?php echo $res['price1'];?>" class="dfinput" /><i>（英文）</i></li>
        <li ><label>启运地</label><input type="text" id="chandi" name="chandi" value="<?php echo $res['chandi'];?>" class="dfinput" /><i>（中文）</i></li>
        <li ><label>启运地</label><input type="text" id="chandien" name="chandien" value="<?php echo $res['chandien'];?>" class="dfinput"  /><i>（英文）</i></li>
        <li ><label>目的地</label><input type="text" id="caizhi" name="caizhi" value="<?php echo $res['caizhi'];?>" class="dfinput" /><i>（中文）</i></li>
        <li ><label>目的地</label><input type="text" id="caizhien" name="caizhien" value="<?php echo $res['caizhien'];?>" class="dfinput"  /><i>（英文）</i></li>
        <li ><label>业务范围</label><input type="text" id="workarea" name="workarea" value="<?php echo $res['workarea'];?>" class="dfinput" /><i>（中文）</i></li>
        <li ><label>业务范围</label><input type="text" id="workareaen" name="workareaen" value="<?php echo $res['workareaen'];?>" class="dfinput"  /><i>（英文）</i></li>
        <li>
            <label>缩略图</label>
        ...
        <li>
            ...
            <i>
                ...
                {
                    ?>
                    <a href="javascript:;" onMouseOver="document.getElementById('pic2').style.display='block';" onMouseOut="document.getElementById('pic2').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="logisticsFileDel.php?id=<?php echo $res['id'];?>&fld=spicen" style="color:#FF0000;">删除</a>]
        ...
        <li style="display: none">
            ...
            <i>
                ...
                {
                ?>
                <a href="javascript:;" onMouseOver="document.getElementById('pic3').style.display='block';" onMouseOut="document.getElementById('pic3').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="logisticsFileDel.php?id=<?php echo $res['id'];?>&fld=bpic" style="color:#FF0000;">删除</a>]
                <div id="pic3" style="position:absolute;z-index:1;display:none;"><img src="../../<?php echo $res['bpic'];?>" width="100" border="0" style="border:1px solid #000000;" /></div>
                <?php
                }
                else
                {
        ...
        <li style="display: none">
            <label>产品大图</label>
            <input type="hidden" id="bpicen" name="bpicen" value="../../<?php echo $res['bpicen'];?>" size="53" class="tx1" />
            <iframe id="UploadFiles" src="../upfile/upload.php?formname=myform&editname=bpicen&uppath=proimg&filelx=jpg" frameborder="0" scrolling="No" width="280" height="30" style="top:2px;"></iframe>
            <i>
                <?php
                if ($res['bpicen'] != "")
                {
                    ?>
                    <a href="javascript:;" onMouseOver="document.getElementById('pic4').style.display='block';" onMouseOut="document.getElementById('pic4').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="logisticsFileDel.php?id=<?php echo $res['id'];?>&fld=bpicen" style="color:#FF0000;">删除</a>]
                    <div id="pic4" style="position:absolute;z-index:1;display:none;"><img src="../../<?php echo $res['bpicen'];?>" width="100" border="0" style="border:1px solid #000000;" /></div>
                    <?php
                }
                else
                {
        ...
        <li id="myTR_<?php echo $i;?>" style="display:none;">
            ...
            <i>
                ...
                {
                ?>
                <a href="logisticsFileDel1.php?id=<?php echo $res['id'];?>&pic=<?php echo $arrImg[$i];?>">删除文件</a>
                <?php
                }
                else
                {
        ...
        ?>
        <li style="display: none"><label>产品参数<i><br />（中文）</i></label><textarea id="text" name="text" cols="" rows="" class="textinput2" style="height:250px;"><?php echo $res['text'];?></textarea></li>
        <li style="display: none"><label>产品参数</label><textarea id="text_en" name="text_en" cols="" rows="" class="textinput2" style="height:250px;"><?php echo $res['text_en'];?></textarea><i>（英文）</i></li>
        <li><label>产品详情<i><br />（中文）</i></label><textarea id="content" name="content" cols="" rows="" class="textinput2"><?php echo $res['content'];?></textarea></li>
        <li ><label>产品详情<i><br />（英文）</i></label><textarea id="content_en" name="content_en" cols="" rows="" class="textinput2"><?php echo $res['content_en'];?></textarea></li>
        <li  style="display: none"><label>产品展示<i><br />（中文）</i></label><textarea id="proshow" name="proshow" cols="" rows="" class="textinput2" style="height:250px;"><?php echo $res['proshow'];?></textarea></li>
        ...
        <li ><label>网页标题</label><input type="text" id="t_en" name="t_en" value="<?php echo $res['t_en'];?>" class="dfinput2" /><i>（英文）</i></li>
        <li ><label>网页关键字</label><input type="text" id="k" name="k" value="<?php echo $res['k'];?>" class="dfinput2" /><i>（中文）</i></li>
        <li ><label>网页关键字</label><input type="text" id="k_en" name="k_en" value="<?php echo $res['k_en'];?>" class="dfinput2" /><i>（英文）</i></li>
        <li ><label>网站描述</label><textarea id="d" name="d" cols="" rows="" class="textinput"><?php echo $res['d'];?></textarea><i>（中文）</i></li>
        <li ><label>网站描述</label><textarea id="d_en" name="d_en" cols="" rows="" class="textinput"><?php echo $res['d_en'];?></textarea><i>（英文）</i></li>
        ...
        <li><label>推荐显示</label><input type="checkbox" id="tj" name="tj" value="1" <?php if ($res['tj'] != 0){echo "checked";} ?> /><i>（如果选中的话将推荐显示）</i></li>
