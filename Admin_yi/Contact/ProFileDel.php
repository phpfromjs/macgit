...
if (!empty($_GET['id']))
{
    $sql = "SELECT * FROM ".$data['contact']." WHERE qy=".$_SESSION['qy']." AND id=".$_GET['id'] ;
    ...
    if (mysql_num_rows($resul) != 0)
    {
        if (!empty($_GET['fld']))
        {
            ...
            if ($_GET['fld'] == "spic")
            {
                $sqll = "UPDATE ".$data['contact']." SET spic='$hpic' WHERE qy=".$_SESSION['qy']." AND id=".$_GET['id'];
            }
            else
            {
                $sqll = "UPDATE ".$data['contact']." SET ".$_GET['fld']."='$hpic' WHERE qy=".$_SESSION['qy']." AND id=".$_GET['id'];
            }
