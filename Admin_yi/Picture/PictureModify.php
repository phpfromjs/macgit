...
if (!empty($_GET['pID']))
{
    $id = $_GET['pID'];
    $page = $_GET['page'];
    $sql = "SELECT * FROM ".$data['picture']." WHERE id='$id'";
...
<div class="formbody">
    ...
    <form method="POST" name="myform" onSubmit="return CheckForm();" action="PictureSave.php?action=Modify">
    <ul class="forminfo">
        ...
        <li ><label>图片链接</label><input type="text" id="model" name="model" value="<?php echo $res['model'];?>" class="dfinput" /><i>（中文）</i></li>
        <li ><label>图片链接</label><input type="text" id="modelen" name="modelen" value="<?php echo $res['modelen'];?>" class="dfinput" /><i>（英文）</i></li>
        <li>
            <label>图片</label>
