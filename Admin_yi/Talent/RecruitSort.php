...
function ShowTree($parentID, $conn, $date)
{
    ...
    while ($rs = mysql_fetch_array($resul))
    {
        ...
        {
            echo "<b style='font-size:14px;'>".$rs['class_name_cn']."</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$adnext.$ednext.$denext.$mvnext."<b class='red'>(".$rs['sort'].")</b></div>";
        }
        else
        {
            echo "<b style='font-size:14px;'>".$rs['class_name_en']."</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$adnext.$ednext.$denext.$mvnext."<b class='red'>(".$rs['sort'].")</b></div>";
        }
...
?>
<?php
$sql = "SELECT * FROM ".$data['classrecruit']." ORDER BY sort DESC";
$resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
...
?>
    ...
    <form name="myform" method="post" action="RecruitSort.php?action=add_myclass" onSubmit="return check();">
    <ul class="forminfo">
    ...
    case "Addnext";
        ...
        $all_id = $_GET['all_id'];
        $level = $_GET['level'];
        $px = "SELECT * FROM ".$data['classrecruit']." WHERE prv_id=".$prv_id." ORDER BY sort DESC";
    ...
    <form name="myform" method="post" action="RecruitSort.php?action=add_myclassNext" onSubmit="return check();">
    ...
    <?php
        ...
        {
            $level = $_GET['level'];
            $sql = "SELECT * FROM ".$data['classrecruit']." WHERE classid=".$pid;
            $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            if (mysql_num_rows($resul) != 0)
            {
   ...
   <form name="myform" method="post" action="RecruitSort.php?action=edit_myclass" onSubmit="return check();">
    ...
    <ul class="forminfo">
        ...
        <li style="display:none;">
            ...
            <i>
                ...
                {
                ?>
                <a href="javascript:;" onMouseOver="document.getElementById('pic1').style.display='block';" onMouseOut="document.getElementById('pic1').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="RecruitSort.php?action=del_files&pid=<?php echo $rs['classid'];?>&amp;fld=class_img" style="color:#FF0000;">删除</a>]
        ...
        <li style="display:none;">
            ...
            <i>
                ...
                {
                ?>
                <a href="javascript:;" onMouseOver="document.getElementById('pic2').style.display='block';" onMouseOut="document.getElementById('pic2').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="RecruitSort.php?action=del_files&pid=<?php echo $rs['classid'];?>&amp;fld=class_img_en" style="color:#FF0000;">删除</a>]
    ...
    case "add_myclass";
        ...
        $d_en = $_POST['d_en'];
        $d_j = $_POST['d_j'];
        $sql = "SELECT * FROM ".$data['classrecruit']." WHERE class_name_cn='".$title."'";
        $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($resul) == 0)
        {
            $string = "INSERT INTO ".$data['classrecruit']."(prv_id,all_id,class_name_cn,class_name_en,class_name_j,sort,class_img,class_img_en,class_img_j,lx,t,t_en,t_j,k,k_en,k_j,d,d_en,d_j,p,p_en,p_j) VALUES('0','0','$title','$title_en','$title_j','$sort','$img','$img_en','$img_j','$lx','{$t}','{$t_en}','{$t_j}','{$k}','{$k_en}','{$k_j}','{$d}','{$d_en}','{$d_j}','{$p}','{$p_en}','{$p_j}')";
            mysql_query($string, $conn) or die("ERROR: ".mysql_error());
            $sq = "SELECT * FROM ".$data['classrecruit']." WHERE class_name_cn='".$title."'";
            ...
            if (mysql_num_rows($pp) != 0)
            {
                $temp = $tt['classid'];
                $temp2 = "|".$tt['classid']."|";
                $sq1 = "UPDATE ".$data['classrecruit']." SET all_id='$temp',link_id='$temp2' WHERE class_name_cn='".$title."'";
                mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
                echo "<script>alert('状态：添加成功');location.href='RecruitSort.php';</script>";
            }
        }
        else
        {
    ...
    case "edit_myclass";
        ...
        $d_en = $_POST['d_en'];
        $d_j = $_POST['d_j'];
        $sq = "SELECT * FROM ".$data['classrecruit']." WHERE classid=".$pid;
        $pp = mysql_query($sq, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($pp) != 0)
        {
            $sq1="UPDATE ".$data['classrecruit']." SET class_name_cn='$title',class_name_en='$title_en',class_name_j='$title_j',sort='$sort',class_img='$img',class_img_en='$img_en',class_img_j='$img_j',lx='$lx',t='$t',t_en='$t_en',t_j='$t_j',k='$k',k_en='$k_en',k_j='$k_j',d='$d',d_en='$d_en',d_j='$d_j',p='$p',p_en='$p_en',p_j='$p_j' WHERE classid=".$pid;
            mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
            echo "<script>alert('修改成功');location.href='RecruitSort.php';</script>";
        }
        else
        {
            echo "<script>alert('没有找到数据!');location.href='RecruitSort.php';</script>";
        }
        break;
    case "del_myclass";
        if (!empty($_GET['pid']))
        {
            //删除当前类
            $rs = "SELECT * FROM ".$data['classrecruit']." WHERE classid=".$_GET['pid'];
            ...
            $sql = "DELETE FROM ".$data['classrecruit']." WHERE classid=".$_GET['pid']; 
            mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            //删除子类
            $rs = "SELECT * FROM ".$data['classrecruit']." WHERE prv_id=".$_GET['pid'];
            $resul = mysql_query($rs, $conn) or die("ERROR: ".mysql_error());
            while ($title = mysql_fetch_array($resul))
            {
                $spic = "../../".$title['class_img'];
            ...
            $sql = "DELETE FROM ".$data['classrecruit']." WHERE prv_id=".$_GET['pid']; 
            mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            echo "<script language='javascript'>window.alert('删除对象成功！'); location='RecruitSort.php';</script>";
        }
        else
        {
            echo "<script language='javascript'>window.alert('数据丢失！'); location='RecruitSort.php';</script>";
        }
        break;
    case "mv_myclass";
        $p_sort = -1;
        ...
        if (!empty($_GET['pid']))
        {
            $resul = mysql_query("SELECT * FROM ".$data['classrecruit']." WHERE classid='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            $tt = mysql_fetch_array($resul);
            if (mysql_num_rows($resul) != 0)
            {
                $p_sort = $tt['sort'];
        ...
        if (!empty($_GET['mID']))
        {
            $resull = mysql_query("SELECT * FROM ".$data['classrecruit']." WHERE classid='{$_GET['mID']}'", $conn) or die("ERROR: ".mysql_error());
        ...
        if (!empty($_GET['mID']))
        {
            if ($p_sort != -1)
            {
                $px = "UPDATE ".$data['classrecruit']." SET sort='{$p_sort}' WHERE classid='{$_GET['mID']}'";
        ...
        if (!empty($_GET['pid']))
        {
            if ($m_sort != -1)
            {
                mysql_query("UPDATE ".$data['classrecruit']." SET sort='{$m_sort}' WHERE classid='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            }
        }
        header("Location:RecruitSort.php");
        break;
    case "del_files";
        if (!empty($_GET['pid']) && is_numeric($_GET['pid']))
        {
            $sql = "SELECT * FROM ".$data['classrecruit']." WHERE classid=".$_GET['pid'];
            $resul = mysql_query($sql, $conn) or die("ERROR:".mysql_error());
            ...
            if (mysql_num_rows($resul) != 0)
            {
                if (!empty($_GET['fld']))
                {
                    ...
                    {
                        $sqll = "UPDATE ".$data['classrecruit']." SET class_img='$hpic' WHERE classid=".$_GET['pid'];
                    }
                    else
                    {
                        $sqll = "UPDATE ".$data['classrecruit']." SET ".$_GET['fld']."='$hpic' WHERE classid=".$_GET['pid'];
                    }
    ...
    case "add_myclassNext";
        ...
        $all_id = $_POST['all_id'];
        $img = str_replace("../../", "", $_POST['class_img']);
        $sql = "SELECT * FROM ".$data['classrecruit']." WHERE class_name_cn='".$title."' AND prv_id=".$prv_id;
        $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($resul) == 0)
        {
            $string = "INSERT INTO ".$data['classrecruit']."(prv_id,all_id,class_name_cn,class_name_en,sort,class_img) VALUES('$prv_id','$all_id','$title','$title_en','$sort','$img')";
            mysql_query($string, $conn) or die("ERROR: ".mysql_error());
            $sq = "SELECT * FROM ".$data['classrecruit']." WHERE classid='".$prv_id."'";
            $pp = mysql_query($sq, $conn) or die("ERROR: ".mysql_error());
            $tt = mysql_fetch_array($pp);
            if (mysql_num_rows($pp) != 0)
            {
            ...
            $sqd = "SELECT * FROM ".$data['classrecruit']." ORDER BY classid DESC";
            ...
            if (mysql_num_rows($ppp) != 0)
            {
                $temp2 = $ttt['classid'];
                $templink = $temp.$temp2."|";
                $sq1 = "UPDATE ".$data['classrecruit']." SET link_id='$templink' WHERE classid=".$temp2;
                mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
                echo "<script>alert('状态：添加成功');location.href='RecruitSort.php';</script>";
            }
        }
