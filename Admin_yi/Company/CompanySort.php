...
?>
<?php
$sql = "SELECT * FROM ".$data['classcompany']." ORDER BY sort DESC";
$resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
...
?>
    ...
    <form name="myform" method="post" action="CompanySort.php?action=add_myclass" onSubmit="return check();">
    <ul class="forminfo">
    ...
    case "Addnext";
        ...
        $all_id = $_GET['all_id'];
        $level = $_GET['level'];
        $px = "SELECT * FROM ".$data['classcompany']." WHERE prv_id=".$prv_id." ORDER BY sort DESC";
    ...
    <form name="myform" method="post" action="CompanySort.php?action=add_myclassNext" onSubmit="return check();">
    ...
    <?php
        ...
        {
            $level = $_GET['level'];
            $sql = "SELECT * FROM ".$data['classcompany']." WHERE classid=".$pid;
            $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            if (mysql_num_rows($resul) != 0)
            {
   ...
   <form name="myform" method="post" action="CompanySort.php?action=edit_myclass" onSubmit="return check();">
    ...
    <ul class="forminfo">
        ...
        <li style="display:none;">
            ...
            <i>
                ...
                {
                ?>
                <a href="javascript:;" onMouseOver="document.getElementById('pic1').style.display='block';" onMouseOut="document.getElementById('pic1').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="CompanySort.php?action=del_files&pid=<?php echo $rs['classid'];?>&amp;fld=class_img" style="color:#FF0000;">删除</a>]
        ...
        <li style="display:none;">
            ...
            <i>
                ...
                {
                ?>
                <a href="javascript:;" onMouseOver="document.getElementById('pic2').style.display='block';" onMouseOut="document.getElementById('pic2').style.display='none';">图片已上传</a>&nbsp;&nbsp;[<a href="CompanySort.php?action=del_files&pid=<?php echo $rs['classid'];?>&amp;fld=class_img_en" style="color:#FF0000;">删除</a>]
    ...
    case "add_myclass";
        ...
        $d_en = $_POST['d_en'];
        $d_j = $_POST['d_j'];
        $sql = "SELECT * FROM ".$data['classcompany']." WHERE class_name_cn='".$title."'";
        $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($resul) == 0)
        {
            $string="INSERT INTO ".$data['classcompany']."(class_txt_cn,class_txt_en,class_txt_j,prv_id,all_id,class_name_cn,class_name_en,class_name_j,sort,class_img,class_img_en,class_img_j,p,t,t_en,t_j,k,k_en,k_j,d,d_en,d_j) VALUES('$class_txt_cn','$class_txt_en','$class_txt_j','0','0','$title','$title_en','$title_j','$sort','$img','$img_en','$img_j','$p','{$t}','{$t_en}','{$t_j}','{$k}','{$k_en}','{$k_j}','{$d}','{$d_en}','{$d_j}')";
            mysql_query($string, $conn) or die("ERROR: ".mysql_error());
            $sq = "SELECT * FROM ".$data['classcompany']." WHERE class_name_cn='".$title."'";
            ...
            if (mysql_num_rows($pp) != 0)
            {
                $temp = $tt['classid'];
                $temp2 = "|".$tt['classid']."|";
                $sq1 = "UPDATE ".$data['classcompany']." SET all_id='$temp',link_id='$temp2' WHERE class_name_cn='".$title."'";
                mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
                echo "<script>alert('状态：添加成功');location.href='CompanySort.php';</script>";
            }
        }
        else
        {
    ...
    case "del_myclass";
        ...
        {
            //删除当前类
            $rs = "SELECT * FROM ".$data['classcompany']." WHERE classid=".$_GET['pid'];
            ...
            $sql = "DELETE FROM ".$data['classcompany']." WHERE classid=".$_GET['pid']; 
            mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            //删除子类
            $rs = "SELECT * FROM ".$data['classcompany']." WHERE prv_id=".$_GET['pid'];
            $resul = mysql_query($rs, $conn) or die("ERROR: ".mysql_error());
            while ($title = mysql_fetch_array($resul))
            {
                $spic = "../../".$title['class_img'];
            ...
            $sql = "DELETE FROM ".$data['classcompany']." WHERE prv_id=".$_GET['pid']; 
            mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
            echo "<script language='javascript'>window.alert('删除对象成功！'); location='CompanySort.php?glprv_id=".$xgprv_id."';</script>";
        }
        else
        {
            echo "<script language='javascript'>window.alert('数据丢失！'); location='CompanySort.php?glprv_id=".$xgprv_id."';</script>";
        }
        break;
    case "mv_myclass";
        $p_sort = -1;
        ...
        if (!empty($_GET['pid']))
        {
            $resul = mysql_query("SELECT * FROM ".$data['classcompany']." WHERE classid='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            $tt = mysql_fetch_array($resul);
            if (mysql_num_rows($resul) != 0)
            {
                $p_sort = $tt['sort'];
        ...
        if (!empty($_GET['mID']))
        {
            $resull = mysql_query("SELECT * FROM ".$data['classcompany']." WHERE classid='{$_GET['mID']}'", $conn) or die("ERROR: ".mysql_error());
        ...
        if (!empty($_GET['mID']))
        {
            if ($p_sort != -1)
            {
                $px = "UPDATE ".$data['classcompany']." SET sort='{$p_sort}' WHERE classid='{$_GET['mID']}'";
        ...
        if (!empty($_GET['pid']))
        {
            if ($m_sort != -1)
            {
                mysql_query("UPDATE ".$data['classcompany']." SET sort='{$m_sort}' WHERE classid='{$_GET['pid']}'", $conn) or die("ERROR: ".mysql_error());
            }
        }
        header("Location:CompanySort.php?xgprv_id=".$_GET['pid']."");
        break;
    case "del_files";
        if (!empty($_GET['pid']) && is_numeric($_GET['pid']))
        {
            $sql = "SELECT * FROM ".$data['classcompany']." WHERE classid=".$_GET['pid'];
            $resul = mysql_query($sql, $conn) or die("ERROR:".mysql_error());
            ...
            if (mysql_num_rows($resul) != 0)
            {
                if (!empty($_GET['fld']))
                {
                    ...
                    {
                        $sqll = "UPDATE ".$data['classcompany']." SET class_img='$hpic' WHERE classid=".$_GET['pid'];
                    }
                    else
                    {
                        $sqll = "UPDATE ".$data['classcompany']." SET ".$_GET['fld']."='$hpic' WHERE classid=".$_GET['pid'];
                    }
    ...
    case "add_myclassNext";
        ...
        $a4 = $_POST['a4'];
        $a4_en = $_POST['a4_en'];
        $sql = "SELECT * FROM ".$data['classcompany']." WHERE class_name_cn='".$title."' AND prv_id=".$prv_id;
        $resul = mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
        if (mysql_num_rows($resul) == 0)
        {
            $string = "INSERT INTO ".$data['classcompany']."(class_txt_cn,class_txt_en,class_txt_j,prv_id,all_id,class_name_cn,class_name_en,class_name_j,sort,class_img,class_img_en,class_img_j,t,t_en,t_j,k,k_en,k_j,d,d_en,d_j,p,a1,a1_en,a2,a2_en,a3,a3_en,a4,a4_en) VALUES('$class_txt_cn','$class_txt_en','$class_txt_j','$prv_id','$all_id','$title','$title_en','$title_j','$sort','$img','$img_en','$img_j','{$t}','{$t_en}','{$t_j}','{$k}','{$k_en}','{$k_j}','{$d}','{$d_en}','{$d_j}','{$p}','{$a1}','{$a1_en}','{$a2}','{$a2_en}','{$a3}','{$a3_en}','{$a4}','{$a4_en}')";
            mysql_query($string, $conn) or die("ERROR: ".mysql_error());
            $sq = "SELECT * FROM ".$data['classcompany']." WHERE classid='".$prv_id."'";
            $pp = mysql_query($sq, $conn) or die("ERROR: ".mysql_error());
            $tt = mysql_fetch_array($pp);
            if (mysql_num_rows($pp) != 0)
            {
            ...
            $sqd = "SELECT * FROM ".$data['classcompany']." ORDER BY classid DESC";
            ...
            if (mysql_num_rows($ppp) != 0)
            {
                $temp2 = $ttt['classid'];
                $templink = $temp.$temp2."|";
                $sq1 = "UPDATE ".$data['classcompany']." SET link_id='$templink' WHERE classid=".$temp2;
                mysql_query($sq1, $conn) or die("ERROR: ".mysql_error());
                echo "<script>alert('状态：添加成功');location.href='CompanySort.php?glprv_id=".$prv_id."';</script>";
            }
        }
        else
        {
