...
if (!empty($allid))
{
    ...
    if (!empty($allidlist))
    {
        $rs = "SELECT * FROM ".$data['company']." WHERE id IN($allidlist)";
        ...
        $sql = "DELETE FROM ".$data['company']." WHERE id IN($allidlist)"; 
        mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
        echo "<script language='javascript'>window.alert('删除对象成功！'); location='CompanyList.php';</script>";
        exit;
    }
}
?>
...
if (!empty($id))
{
    $rs = "SELECT * FROM ".$data['company']." WHERE id='".$id."'";
    $resul = mysql_query($rs, $conn) or die("ERROR: ".mysql_error());
    while ($title = mysql_fetch_array($resul))
    {
        $spic = "../../".$title['spic'];
    ...
    $sql = "DELETE FROM ".$data['company']." WHERE id='".$id."'";
    mysql_query($sql, $conn) or die("ERROR: ".mysql_error());
    echo "<script language='javascript'> window.alert('删除对象成功！'); location='CompanyList.php';</script>";
    exit;
}
